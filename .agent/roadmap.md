...
| M4 | Multi-agent quality | P4 | 🟡 mostly — Debugger (ADR-015), Browser QA (ADR-016), Memory Curator (ADR-017), and **Reviewer** (ADR-018: read-only adversarial diff review, `local-ezai review`) shipped; Context/Research agent + reviewer-in-run-pipeline gate still open | measurable quality delta on 10-task fixture suite; reviewer catches seeded regression |
...
| M6 | Interfaces & A3 delivery | P6 | 🟡 partially — production CLI `local-ezai` shipped (ADR-018: 10 commands, path-first UX, chat REPL, sprint execution, cross-platform packaging; 222 tests). Web console, chat-ops MCP tools, PR/forge delivery still open | console gate approvals; chat-ops tools; PR opened on LAN forge at A3 |
