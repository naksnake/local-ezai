...
|---|---|---|
| `model install <ref> [--name N] [--runtime R] [--group G] [--refetch]` | `<ref>` = `hf:<org/repo>` · `gguf:<url \| hf://org/repo/file.gguf \| path>` · catalog id · `auto` (needs `--group`); fetch → side-load validation → `installed` / `failed`. Without `--runtime`, an `hf:`/`gguf:` source is served by the **active slot's runtime** when it serves the format, else by the first descriptor that does (PR-25) | no (audited) |
...
| `memory [--search] [--limit]` · `memory --add TEXT --kind K` (repository work, always in-process — the Admin Center's Memory page uses these) | `GET /v1/projects/{name}/memory?kind=&search=&limit=` (`project_memory`) · `POST /v1/projects/{name}/memory` `{kind, text}` (`project_memory_add`) — contract **1.1.0** (PR-19) |
| `setup` (writes `config/first-run/report.json`; the Platform-ready card) | `GET /v1/first-run` (`first_run_report`) — contract **1.2.0** (PR-23) |

The contract is **1.2.0**: the 29 operations frozen at 1.0.0 (P2 close),
plus the two memory operations (1.1.0) and the first-run report (1.2.0),
additively — `docs/api/ezaid-openapi.json`, regenerated by `make
control-spec` and inventory-pinned in the suite.
